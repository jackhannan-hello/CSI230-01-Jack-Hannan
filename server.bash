...
#Check if wg0.conf exist
if [[ -f"wg0.conf" ]]
then
        #promt if want to overwrite file
        echo "The file wg0.conf already exits."
        echo -n "Do you want to overwrite it? [y|N]"
        read want_overwrite

        if [[ "${want_overwrite}" == "N" || "${to_overwrite}" == "n" ]]
        then
                echo "Ok Exiting"
                exit 0
        elif [[ "${want_overwrite}" == "y" || "${want_overwrite}" == "Y" ]]
        then
                echo "Creating a new wiregaurd configuration file"
